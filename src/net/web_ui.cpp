...
:root{--bg:#fff7ea;--bg2:#ffe3c7;--panel:rgba(255,255,255,.86);--panelStrong:rgba(255,255,255,.94);--line:rgba(255,168,108,.22);--text:#1d2430;--muted:#625c56;--accent:#4fe38d;--accentDeep:#00b86b;--accentSoft:#dffceb;--warn:#ffbf47;--err:#ff6f61;--glow:rgba(255,135,82,.22);--shadow:0 22px 44px rgba(146,82,27,.14);--shadowSoft:0 12px 28px rgba(146,82,27,.1);}
...
radial-gradient(circle at 18% 14%, rgba(0,205,140,.18) 0%, rgba(0,205,140,0) 28%),
radial-gradient(circle at 86% 18%, rgba(255,132,74,.25) 0%, rgba(255,132,74,0) 32%),
linear-gradient(180deg,#fffdf7 0%,#fff1dd 42%,#ffe4cb 100%);}
body::before,body::after{content:"";position:fixed;z-index:0;border-radius:999px;pointer-events:none;filter:blur(14px);}
body::before{width:240px;height:240px;top:92px;left:-72px;background:radial-gradient(circle, rgba(255,199,72,.32) 0%, rgba(255,199,72,0) 70%);}
body::after{width:260px;height:260px;right:-96px;bottom:110px;background:radial-gradient(circle, rgba(0,184,107,.2) 0%, rgba(0,184,107,0) 72%);}
body.state-A{--accent:#9bc7f4;--accentDeep:#4093d8;--accentSoft:#d9ecff}
body.state-B{--accent:#ffd55a;--accentDeep:#f0a700;--accentSoft:#fff1b8}
body.state-C,body.state-D{--accent:#4fe38d;--accentDeep:#00b86b;--accentSoft:#dffceb}
body.state-E,body.state-F{--accent:#ff8c72;--accentDeep:#ff5a3d;--accentSoft:#ffd8d0}
.app{position:relative;z-index:1;max-width:430px;margin:0 auto;padding:18px 18px 28px;}
.topbar{display:grid;grid-template-columns:44px 1fr 52px;align-items:center;gap:10px;}
.backBtn{width:44px;height:44px;border-radius:16px;display:grid;place-items:center;text-decoration:none;color:#182130;font-size:30px;line-height:1;background:linear-gradient(180deg,rgba(255,255,255,.92),rgba(255,244,229,.76));border:1px solid rgba(255,163,103,.22);box-shadow:var(--shadowSoft);}
.statusWrap{display:flex;justify-content:center;}
.statusPill{display:inline-flex;align-items:center;gap:10px;padding:11px 16px;border-radius:999px;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(255,245,232,.9));border:2px solid color-mix(in srgb,var(--accentDeep) 46%, white);box-shadow:0 16px 30px color-mix(in srgb,var(--accentDeep) 18%, rgba(146,82,27,.16));font-weight:800;font-size:14px;letter-spacing:.01em;color:#202734;}
.statusIcon{width:24px;height:24px;display:grid;place-items:center;color:var(--accentDeep);}
...
.pillDash{opacity:.55}
.sync{justify-self:end;min-width:52px;height:36px;padding:0 10px;border-radius:999px;background:linear-gradient(180deg,rgba(255,255,255,.94),rgba(255,240,222,.8));border:1px solid rgba(255,170,110,.28);display:flex;align-items:center;justify-content:center;gap:7px;font-size:11px;font-weight:800;letter-spacing:.08em;color:#8a6548;text-transform:uppercase;box-shadow:var(--shadowSoft);}
.sync::before{content:"";width:8px;height:8px;border-radius:999px;background:#f6c786;box-shadow:0 0 0 4px rgba(246,199,134,.24);}
.sync.live{color:#0b8d55;border-color:rgba(0,184,107,.28);background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(223,252,235,.9));}
.sync.live::before{background:var(--accentDeep);box-shadow:0 0 0 4px rgba(0,184,107,.16);}
.headline{padding:14px 8px 0;text-align:center;}
.dateLine{font-size:18px;font-weight:700;letter-spacing:.01em;color:#21405d;}
.locationLine{margin-top:10px;font-size:18px;line-height:1.35;font-weight:700;color:#273142;}
.hintLine{margin-top:6px;font-size:13px;color:#76543c;}
.carStage{margin-top:20px;}
.carWrap{position:relative;max-width:330px;margin:0 auto;display:flex;align-items:center;justify-content:center;}
.carHalo{position:absolute;inset:8% 6% 12%;border-radius:48px;background:radial-gradient(circle at 50% 48%, rgba(255,255,255,.88) 0%, color-mix(in srgb,var(--accent) 62%, white) 26%, rgba(255,165,85,.35) 52%, rgba(255,255,255,0) 82%);filter:blur(10px);opacity:.98;}
.carFill{position:absolute;inset:0;--fillColor:rgba(124,210,112,.86);--alpha:0;--car-img:none;-webkit-mask-image:var(--car-img);mask-image:var(--car-img);-webkit-mask-size:contain;mask-size:contain;-webkit-mask-repeat:no-repeat;mask-repeat:no-repeat;-webkit-mask-position:center;mask-position:center;overflow:hidden;opacity:var(--alpha);transition:opacity .7s ease;z-index:1;}
.carFillInner{position:absolute;left:0;right:0;bottom:0;height:0%;background:linear-gradient(180deg,color-mix(in srgb,var(--accent) 52%, white) 0%, var(--accent) 46%, var(--accentDeep) 100%);box-shadow:0 0 28px color-mix(in srgb,var(--accentDeep) 30%, transparent) inset;transition:height .55s ease,background-color .55s ease;}
.carFill.stateA{--alpha:0}
.carFill.stateB{--alpha:.84;--fillColor:rgba(255,208,86,.92)}
.carFill.stateC,.carFill.stateD{--alpha:1;--fillColor:rgba(79,227,141,.94)}
.carFill.stateC .carFillInner,.carFill.stateD .carFillInner{animation:carChargeFill 5.8s ease-in-out infinite;}
.carFill.stateE,.carFill.stateF{--alpha:1;--fillColor:rgba(255,111,97,.95)}
.carFill.stateE .carFillInner,.carFill.stateF .carFillInner{animation:carFaultPulse 1.1s ease-in-out infinite;}
.carSvg{position:relative;z-index:2;width:100%;max-width:330px;height:auto;display:block;filter:drop-shadow(0 18px 24px rgba(24,33,48,.18));}
.batteryRow{margin-top:16px;display:flex;align-items:center;gap:10px;padding:14px 16px;border-radius:24px;background:linear-gradient(180deg,rgba(255,255,255,.9),rgba(255,247,236,.78));border:1px solid rgba(255,171,111,.22);box-shadow:var(--shadowSoft);font-size:22px;font-weight:700;letter-spacing:-.01em;}
...
.batteryIcon{width:28px;height:28px;display:grid;place-items:center;color:#f38b1a;}
...
.metricCard{margin-top:18px;padding:20px 22px;border-radius:26px;background:linear-gradient(180deg,var(--panelStrong),var(--panel));border:1px solid var(--line);box-shadow:var(--shadow);backdrop-filter:blur(10px);}
.metricLabel{font-size:16px;color:#815943;font-weight:700;}
.metricHero{margin-top:10px;font-size:44px;line-height:1;font-weight:800;letter-spacing:-.04em;color:#18324d;}
.metricHero .unit{font-size:.58em;font-weight:700;letter-spacing:-.01em;color:#325169;}
.metricsGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:22px 18px;}
.metricCell{min-width:0;}
.metricHead{display:flex;align-items:center;gap:8px;color:#705544;font-size:16px;font-weight:700;}
...
.metricValue{margin-top:9px;font-size:20px;font-weight:800;letter-spacing:-.03em;color:#223049;}
.metricValue .unit{font-size:.82em;color:#39556b;}
.phaseMeta{margin-top:16px;padding-top:14px;border-top:1px solid rgba(255,173,113,.18);font-size:13px;color:#7b5e48;display:flex;justify-content:space-between;gap:10px;}
.footerStatus{margin-top:18px;padding:16px 18px 18px;border-radius:24px;background:linear-gradient(180deg,rgba(255,255,255,.88),rgba(255,245,232,.78));border:1px solid rgba(255,173,113,.2);box-shadow:var(--shadowSoft);}
.statusRow{display:flex;justify-content:space-between;align-items:center;gap:12px;}
.statusBadge{display:inline-flex;align-items:center;gap:8px;font-size:13px;font-weight:800;color:#31445a;}
.statusOrb{width:10px;height:10px;border-radius:999px;background:#1ecb74;box-shadow:0 0 0 5px rgba(30,203,116,.14);}
.statusOrb.warn{background:#f3a500;box-shadow:0 0 0 5px rgba(243,165,0,.16);}
.statusOrb.err{background:#ff5a3d;box-shadow:0 0 0 5px rgba(255,90,61,.16);}
.statusMeta{font-size:12px;color:#7a5e47;text-align:right;}
.statusText{margin-top:10px;font-size:18px;font-weight:800;line-height:1.3;color:#202b39;}
.statusFoot{margin-top:12px;display:grid;gap:6px;font-size:12px;color:#7a5e47;}
.debugStrip{display:none;}
.installBtn{display:none;margin-top:14px;width:100%;padding:14px 16px;border-radius:18px;border:1px solid rgba(255,154,97,.24);background:linear-gradient(135deg,#fff8ef,#ffe7cf);color:#7c4722;font-size:14px;font-weight:800;box-shadow:var(--shadowSoft);}
...
<div class="app">
  <div class="topbar">
    <a class="backBtn" href="/admin" aria-label="Admin">&#8592;</a>
    <div class="statusWrap">
      <div class="statusPill" id="statusPill">
        ...
        <span class="pillDash">-</span>
        <span class="vehicleName">TOGG T10X</span>
      </div>
    </div>
  ...
  <section class="metricCard">
    <div class="metricsGrid">
      ...
      <div class="metricCell">
        ...
        <div class="metricValue"><span id="voltageMetric">0.0</span> <span class="unit">V</span></div>
      </div>
      <div class="metricCell">
        <div class="metricHead">
...
function pull(){
  fetch('/status',{cache:'no-store'}).then(r=>r.json()).then(d=>{
    ...
    const displayVoltage=phaseCount>=3 ? 400.0 : 230.9;
    ...
    setText('voltageMetric',displayVoltage.toFixed(1));
